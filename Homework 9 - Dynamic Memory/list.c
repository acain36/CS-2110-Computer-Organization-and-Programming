...
static struct user *create_user(char *name, enum user_type type, union user_data data)
{
    ...
    //Initializing the Data for the New User
    ...
    if (type == INSTRUCTOR) {
      ...
      struct instructor *myInstructor;
      myInstructor = (struct instructor *) malloc(sizeof(struct instructor));
      ...
      int test = create_instructor(data.instructor.salary, myInstructor);
      if (test == 1) {
      ...
      (*myUser).data.instructor = *myInstructor;

      free(myInstructor);

    } else if (type == STUDENT) {   //If the Type is STUDENT

      struct student *myStudent;
      myStudent = (struct student *) malloc(sizeof(struct student));
      ...
      int test = create_student(data.student.num_classes, data.student.grades, myStudent);
...
static int user_equal(const struct user *user1, const struct user *user2)
{
  ...
  enum user_type type1 = (*user1).type;
  enum user_type type2 = (*user2).type;
  if(type1 != type2) {
    return 0;
  }
  ...
  if(type1 == INSTRUCTOR && type2 == INSTRUCTOR) {
    if(strcmp((*user1).name,(*user2).name) == 0) {
...
int push_front(struct user_list *list, char *name, enum user_type type, union user_data data)
{
  ...
  struct user *newUser;
  newUser = create_user(name, type, data);
...
int contains(struct user_list *list, struct user *data, struct user **dataOut)
{
  ...
  for(int i = 0; i <(*list).size; i++) {
    if(user_equal(currentUser, data) == 1) {
      *dataOut = currentUser;
      return 0;
    }
    currentUser = (*currentUser).next;
...
void empty_list(struct user_list *list){
  ...
  if(list == NULL || (*list).size <= 0) {
    ...
    int index = 0;
    while (currentUser != NULL && index < (*list).size) {
      tempUser = (*currentUser).next;
      if((*currentUser).type == STUDENT) {
        free((*currentUser).data.student.grades);
      }
...
int get_highest_paid(struct user_list *list, struct user **dataOut)
{
  ...
  for(int i = 0; i <= (*list).size; i++) {
    if((*currentUser).type == INSTRUCTOR) {
      if((*currentUser).data.instructor.salary > highestSalary + 1) {
        highestPaid = currentUser;
        highestSalary = (*currentUser).data.instructor.salary;
      }
    }
    currentUser = (*currentUser).next;
  }
...
int is_passing_all_classes(struct user_list *list, char *name, int *dataOut)
{
  ...
  *dataOut = -1; 
  ...
  for(int i = 0; i < (*list).size; i++) {
    ...
    if((*currentUser).type == STUDENT && (strcmp((*currentUser).name, name) == 0)) {
      if((*currentUser).data.student.num_classes == 0) {
        *dataOut = 0;
        break;
      }
      *dataOut = 1;
      double *studentGrades;
      studentGrades = (*currentUser).data.student.grades;
      for(int j = 0; j < (*currentUser).data.student.num_classes; j++) {
        if(studentGrades[j] < 60) {
          *dataOut = 0;
          break;
        }
      }
...
int end_semester(struct user_list *list)
{
  ...
  for(int i = 0; i < (*list).size; i++) {
    if((*currentUser).type == STUDENT) {
      ...
      (*currentUser).data.student.num_classes = 0;
      free((*currentUser).data.student.grades);
      (*currentUser).data.student.grades = NULL;

    } else if ((*currentUser).type == INSTRUCTOR) {
      (*currentUser).data.instructor.salary += 10000.0;
    }
